...
- The sidebar-anchored Console surface is the part a user cannot hide. That now rests on the shipped desktop application rather than on the daemon-served web bundle: `dir-m6.35` drove `Paseo-0.7.2-x64.tar.gz` (sha256 `94a3becaecb0aca20f9a76d71dc3e6f797c89f466ae4ed37c81d7f3584611562`) and `Paseo-0.6.1-x64.tar.gz` (sha256 `5c4a0d4a1ed52b25e525c333c6ab95f1b64c27df02040d40c5a688254711ad13`) with the `tools/desktop-evidence` harness. On both builds the sidebar item registers from the plugin registration, opens the Console, and offers no affordance that hides it: it has no context menu, and neither the `Appearance` nor the `Layout` application settings expose a sidebar-item visibility control. The Console's pane has a close control; using it leaves the sidebar item in place, and the item reopens the Console with every tab.
- Removing it is a different act, and the guarantee is scoped to that rather than stated absolutely. On both builds `Settings` → `Host` → `Plugins` lists Director with `Disable` and `Remove` beside a global `Enable plugins` switch, and any of the three takes the sidebar item away — together with the whole of Director. So the guarantee is about Director's own layout while Director is installed and enabled: within that condition the layout cannot be hidden or lost, and uninstalling Director is not losing it.
