...
- Agents receive a session-scoped custom Director MCP in addition to only those applicable built-in Paseo capabilities admitted by the frozen provider policy. Every authenticated agent in a Director-managed Project may also acquire the separate closed `director.project-admin-mcp/v1` fixed to its own Project; this does not widen its Run-scoped role MCP (ADR-0024).
...
- Each Run bridge is fixed to one Project, Task, Run, role, and capability scope. The separate Project-admin bridge is fixed to one Project plus trusted native Workspace, Agent, session, and audience facts and contains no Task/Run/Candidate authority (ADR-0024).
...
`1.0` ships that one runtime TaskStore, not interchangeable engines. ADR-0004 as amended by ADR-0022 requires exact listener/database identity, a bootstrap-owned global safe value, and runtime session/global readback before every write without global runtime administrator authority; ADR-0012 requires Git and Dolt synchronization to remain two visible, separately retryable effects plus verified backup/restore/migration and metrics-safe cleanup. Any identity, authority-attestation, safe-mode, restore, migration, or partial-sync ambiguity fails closed without a fallback store.
...
Any authenticated Paseo agent in a Director-managed Project can acquire the closed typed Project-administration MCP fixed to that one Project (ADR-0024). This amends the former role restriction; no pre-designated administrator or Director-launched origin is required. Its ordinary Task/Epic/Workspace/Run targets are checked for membership and it has no Project selector, raw SDK/storage operation, human-only confirmation, or lifecycle-effect authority. Task Agents retain their separate least-privilege Task/Run tools, and Reviewer Agents retain read-only Candidate/review access plus one structured-verdict operation; those Run-scoped role contracts are not widened.
...
- No affordance this Task searched hides the sidebar-anchored Console surface, and the search is recorded rather than generalised into a universal. `dir-m6.35` drove `Paseo-0.7.2-x64.tar.gz` (sha256 `94a3becaecb0aca20f9a76d71dc3e6f797c89f466ae4ed37c81d7f3584611562`) and `Paseo-0.6.1-x64.tar.gz` (sha256 `5c4a0d4a1ed52b25e525c333c6ab95f1b64c27df02040d40c5a688254711ad13`) with the `tools/desktop-evidence` harness, replacing the daemon-served web bundle this section used to rest on. On both builds the sidebar item registers from the plugin registration and opens the Console. Searched: the item itself, which has no context menu; the `Appearance` and `Layout` application settings, neither of which exposes a sidebar-item visibility control; and the Console's own pane close control, which leaves the sidebar item in place, from where it reopens the Console with every tab. One qualification on `Appearance`: the run enumerated seven controls there on both builds and read an accessible name for six, the seventh being a switch whose own node carries none. The retained screenshot of that pane shows the two switches it contains, `Always expand reasoning` off and `Chat outline` on, matching the enumeration's order and states, so the unnamed control is a chat-rendering preference — read from a screenshot rather than from the control, which is why `dir-m6.39` names it from the application. Not searched: the `General`, `Editor` and `Shortcuts` settings panes, which the same run enumerated and never opened. A visibility control in one of those three would falsify this bullet, so it claims only the places searched until `dir-m6.39` enumerates the domain from the running application instead of from a list.
- Removing Director is a different act from hiding its layout, so the guarantee is scoped rather than stated absolutely. On both builds `Settings` → `Host` → `Plugins` lists Director as a running plugin with `Disable` and `Remove` controls, beside a global `Enable plugins` switch. Those controls were observed; none was pressed, so this section records that host plugin administration offers them and asserts nothing about what pressing one does. `dir-m6.39` presses `Disable` against a harness-owned daemon. What Director promises is therefore its own layout while Director is installed and enabled; host plugin administration sits outside that condition rather than inside it.
...
| Exact-0.7.2 plugin authority is broad | Use only Paseo's handler-scoped public API; never copy a daemon credential or open a second connection | Narrow only after Paseo exposes a supported plugin-scoped capability mechanism; no engine credential or connector fallback |
